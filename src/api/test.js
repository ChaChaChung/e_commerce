...
const getLeagueLastEventsData = async (selectedDate) => {
  try {
    const response = await axios.get(`${API_BASE_URL}/get/league_last_events/${selectedDate}`);
    console.log('API Response:', response.data);
    return response.data;
  } catch (error) {
    console.error('Error fetching test data:', error);
